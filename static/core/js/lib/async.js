 ...
 * @param {*} callbackFunction - response can be rendered with your own callback function
 */
export function Fetcher(request, containerId, mapper = {}, callbackFunction = null) {
    ...
    function formErrors(errors) {
        const type = $A.base.type($A.base.parse(errors));
        ...
        if (type === 'list') {
            errors.forEach((error) => {
                ...
                li.textContent = ' > ' + $A.forms.escapeHtml(($A.base.stringify(error)));
        ...
        if (type === 'string') {
            let li = $A.dom.makeDomElement('li', 'ms-1 text-emphasis-1');
            li.textContent = ' > ' + $A.forms.escapeHtml($A.base.stringify(errors));
            response.appendChild(li);
        }
        return response;
    }
    ...
    function formErrorMessage(messages) {
        const type = $A.base.type($A.base.parse(messages));
        let response = $A.dom.makeDomElement('div', 'text-emphasis-1');
        if (type === 'list') {
            messages.forEach((message) => {
                let span = $A.dom.makeDomElement('span').textContent = $A.forms.escapeHtml(($A.base.stringify(message)));
        ...
        if (type === 'string') {
            response.textContent = $A.forms.escapeHtml($A.base.stringify(messages));
        }
        return response;
    }
...
export function defineRequest(urlKey, urlParams = {}, options = {}) {
    ...
    const finalOptions = $A.base.merge(defaults, options);
    return new Request(url, finalOptions);
}

function selectUrlTemplate(urlKey) {
    if ($A.base.not(urlKey, 'string')) {
...
function generateUrl(template, params) {
    const paramType = $A.base.type(params);
    if (paramType === 'string') {
        let param = params;
        params = {
            input1: param
    ...
    if ($A.base.not(params, 'dictionary')) {
